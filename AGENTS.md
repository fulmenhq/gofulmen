...
- Confirm your operating role (Crucible v0.3.0). Roles are selected from the synced catalog in `config/crucible-go/agentic/roles/`.
...
Gofulmen uses Crucible v0.3.0 role prompts synced from SSOT.
...
2. `Co-Authored-By:` trailer with model email
3. `Role:` trailer matching the active role
4. `Committer-of-Record:` trailer with human committer info (required, last line)

**All four trailers are mandatory. Do not omit any.**
...
Replace `<Model>` with canonical model name (e.g. `Claude Opus 4.6`, `GPT-5.2`), `<Interface>` with agentic tool (e.g. `Claude Code`, `OpenCode`), and `<role>` with the active role slug (e.g. `devlead`).

**DO NOT**:

- Omit any of the four required trailers — especially `Role:` and `Committer-of-Record:`
...
```
fix: prevent config telemetry stdout emission

Config loading now uses the global telemetry system (disabled-by-default),
supports per-call injection, and includes a regression test to keep stdout clean.

Generated by GPT-5.2 via OpenCode under supervision of @3leapsdave
