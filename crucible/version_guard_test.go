...
func TestCrucibleVersionMatchesMetadata(t *testing.T) {
	...
	if normalizedGoMod != normalizedMetadata {
		...
		msg.WriteString("\n")
		fmt.Fprintf(&msg, "go.mod requires:     github.com/fulmenhq/crucible %s\n", goModVersion)
		fmt.Fprintf(&msg, "metadata synced:     %s\n", metadataVersion)
		msg.WriteString("\n")
		msg.WriteString("This means your embedded assets (docs/schemas/configs) are from\n")
		fmt.Fprintf(&msg, "Crucible %s but runtime imports will use %s\n", metadataVersion, goModVersion)
		msg.WriteString("\n")
		msg.WriteString(strings.Repeat("═", 68) + "\n")
		...
		msg.WriteString("\n")
		fmt.Fprintf(&msg, "  go get github.com/fulmenhq/crucible@%s\n", metadataVersion)
