...
Logging Redaction Middleware + Repository Root Discovery

- **Logging Redaction**: Schema-compliant PII and secrets redaction per Crucible v0.2.16
- **Repository Root Discovery**: Fixed schema validation from test subdirectories
- **Pathfinder Enhancement**: Foundation for tooling requiring repository context

### v0.1.16 (Released)

Crucible v0.2.17 Support – DevSecOps Secrets Schema Sync

- **Enhanced Secrets Schema**: Structured credential objects with lifecycle metadata
- **Rotation Policies**: Automated credential rotation configuration
- **Schema Compliance**: Full alignment with Crucible v0.2.17 DevSecOps taxonomy
